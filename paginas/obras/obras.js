    ...
    try {
        ...
        container.innerHTML = `
            <div class="error-message">
                <p>Erro ao carregar os livros. Por favor, tente novamente.</p>
            </div>
        `;
    } finally {
        isLoading = false;
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Adicionar estado de loading na busca
    const estadoBusca = {
        anterior: '',
        loading: false
    };

    // Atualizar a função de busca para mostrar loading
    async function realizarBusca(termo) {
        ...
        // Adicionar classe de loading no container de busca
    ...
    // Atualizar o debounce para usar a nova função de busca
    const debouncedSearchWithLoading = debounce((value) => {
        realizarBusca(value);
    }, 500);

    searchInput.addEventListener('input', (e) => {
        const searchTerm = e.target.value;
        debouncedSearchWithLoading(searchTerm);
    });
});
